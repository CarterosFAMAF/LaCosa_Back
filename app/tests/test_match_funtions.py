...
def test_set_winners():
    ...
    player2.role = PLAYER_ROLE_DEAD
    flush()
    set_winners(match.id, WS_STATUS_HUMANS_WIN)
    assert player1.winner == True
    assert player2.winner == False
    
    player1.role = PLAYER_ROLE_THE_THING
    player2.role = PLAYER_ROLE_DEAD
    flush()
    set_winners(match.id, WS_STATUS_INFECTEDS_WIN)
    assert player1.winner == True
    assert player2.winner == False
    ...
    player2.role = PLAYER_ROLE_INFECTED
    flush()
    set_winners(match.id, WS_STATUS_THE_THING_WIN)
    assert player1.winner == True
    assert player2.winner == False